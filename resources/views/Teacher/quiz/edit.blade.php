<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <nav class="text-sm font-semibold text-gray-500">
            ...
            <span class="mx-2">/</span>
            <span class="text-black">EDIT QUIZ</span>
        </nav>
    </x-slot>
                    ...
                    <form id="course-form" action="{{ route('update.quiz', $quiz->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex items-center mb-6">
                            <div id="icon-preview" class="w-24 h-24 border-dashed border-2 border-gray-400 flex items-center justify-center rounded-full overflow-hidden">
                                <img id="icon-image" src="{{ $quiz->image ? Storage::url($quiz->image) : '' }}" alt="ICON {{ $quiz->title }}" class="{{ $quiz->image ? '' : 'hidden' }} w-full h-full object-cover">
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" id="name" value="{{ old('title', $quiz->title) }}" class="w-full border border-gray-300 rounded-md p-2" placeholder="Write your quiz name" required>
                            @error('title')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="textarea" cols="30" rows="10" class="w-full border border-gray-300 rounded-md p-2" placeholder="Write description for your quiz">{{ old('description', $quiz->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
